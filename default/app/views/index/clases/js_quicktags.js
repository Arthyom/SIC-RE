...
function edToolbar(which) {
	...
	if (edShowExtraCookie()) {
		document.write(
			'<input type="button" id="ed_close_' + which + '" class="ed_button" onclick="edCloseAllTags(\'' + which + '\');" value="Cerrar Tags" />'
...
function edInsertImage(which) {
    myField = document.getElementById(which);
	var myValue = prompt('Ingrese la dirección de la imagen', 'http://');
	if (myValue) {
		myValue = '<img src="' 
				+ myValue 
				+ '" alt="' + prompt('Ingrese la descripción de la imagen', '') 
				+ '" />';
