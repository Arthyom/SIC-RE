...
if (keynum == 8 || keynum == 46 || tabkey == 9)
return true;
...
{
   formulario.Clave.value = formulario.Clave.value;//window.btoa(formulario.Clave.value);
...
function validarusuariosclave2(formulario)
{
   ...
   formulario.Clave.value = formulario.Clave.value;
...
function ValidaAlfanumerico(Cadena)
 {
   	if(!Cadena.match(/^[a-zA-Z0-9_ .-/�������]+$/) && Cadena.length>0) 
     {
			return (false);
		 }
		 return (true);
...
function ValidaCaracteres(Cadena)
 {
   	if(!Cadena.match(/^[a-zA-Z �������]+$/) && Cadena.length>0) 
     {
			return (false);
		 }
		 return (true);
...
function ValidarFecha(Cadena)
 {
	   ...
	   // extrae el a�o el mes y el dia de la fecha
...
function jBusca3(miurl)
  {
     NuevaVentana3= window.open(miurl,'modal','scrollbars=yes, height=430, width=640, modal=yes');
